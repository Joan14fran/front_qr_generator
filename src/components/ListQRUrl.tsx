...
import { getQRsUrl, updateQRUrl, deleteQRUrl } from '../api/qrs.api';
...
export function ListQRUrl() {
    ...
    const updateQRHandler = async () => {
        if (selectedQR) {
            try {
                ...
                await updateQRUrl(selectedQR.id, updatedQR);
                toast.current?.show({
                    severity: 'success',
                    summary: 'Éxito',
    ...
    const deleteQRHandler = async () => {
        if (selectedQR) {
            try {
                await deleteQRUrl(selectedQR.id);
                toast.current?.show({
                    severity: 'info',
                    summary: 'Éxito',
